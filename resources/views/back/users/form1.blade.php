...
<link rel="stylesheet" href="{{ asset('css/form2.css') }}" media="screen" type="text/css" />
        <div id="container">
            ...
            @if(session('ok'))
                ...
                @endif
            <h1>Add user</h1>
            <form action="{{ route('users.indexupdate', $user->id) }}" method="POST">
                @csrf 
                @method('PUT')
                <label><b>Name</b></label>
                <input type="text" placeholder="Enter the name(s)" name="name" value="{{ $user->name }}" required class=" @error('email') is-invalid @enderror" autocomplete="name" autofocus>
                @error('email')
                ...
                <input type="email" placeholder="Enter email" name="email" required autocomplete="email" value="{{ $user->email }}">
